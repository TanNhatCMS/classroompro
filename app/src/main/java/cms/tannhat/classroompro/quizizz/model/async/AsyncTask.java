package cms.tannhat.classroompro.quizizz.model.async;
...
public abstract class AsyncTask<INPUT, PROGRESS, OUTPUT> {
    ...
    /**
     * Starts is all
     * @param input Model you want to work with in the background
     */
    public AsyncTask<INPUT, PROGRESS, OUTPUT> execute(final INPUT input) {
        onPreExecute();
        AtomicReference<OUTPUT> getResult = new AtomicReference<>();
        executorService = AsyncWorker.getInstance().getExecutorService();
        executorService.execute(() -> {
            try {
               getResult.set(doInBackground(input));
                    activity.runOnUiThread(() -> onPostExecute(getResult.get()));
